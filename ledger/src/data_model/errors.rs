...
impl fmt::Display for PlatformError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            PlatformError::InputsError(location) => {
                f.write_str(&format!("Error at: {}", location))
            }
            PlatformError::PolicyFailureError(x) => {
                f.write_str(&format!("Failed policy check: {}", x))
            }
            PlatformError::InvariantError(msg) => {
                f.write_str(&format!("Invariant violated: {}", msg))
            }
            PlatformError::SubmissionServerError(msg) => {
                f.write_str(&format!("Ledger Application Error: {}", msg))
            }
            PlatformError::QueryServerError(msg) => {
                f.write_str(&format!("Query Server Error: {}", msg))
            }
            PlatformError::ZeiError(msg, ze) => f
                .write_str(&format!("Zei error ({}): ", msg))
                .and_then(|_| ze.fmt(f)),
