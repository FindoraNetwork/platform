...
pub fn policy_check_txn(
    type_code: &AssetTypeCode,
                    ...
                    match pending {
                        ...
                        RealTxnOp::Issue(pending_rt, ref mut pending_dsts) => {
                            ...
                            if *pending_rt != *rt {
                                ...
                                used_resources.extend(pending_outputs.drain());
                                pending_op =
                                    Some(RealTxnOp::Issue(*rt, vec![(*amt, *res)]));
                            } else {
                ...
                if let Some(ref mut pending) = pending_op {
                    match pending {
                        RealTxnOp::Issue(_, _) => {
                            ...
                            used_resources.extend(pending_outputs.drain());
                            pending_op =
                                Some(RealTxnOp::Transfer(vec![(*amt, *inp, *out)]));
                        }
        ...
        match (target, real) {
            (RealTxnOp::Issue(_, outs), Operation::IssueAsset(iss)) => {
                ...
                res_vars.extend(
                    outs.iter()
                        .map(|(_, rv)| *rv)
                        .zip(iss.body.records.iter().map(|(rec, _)| rec).cloned()),
                );
    ...
    /* Step 3: Calculate fraction & amount ops */
    {
        ...
        while frac_ix < check.fraction_ops.len() || amt_ix < check.amount_ops.len() {
            ...
            debug_assert!(
                needed_frac_ix == 0
                    || needed_frac_ix
                        < frac_vars.len() + (check.fraction_ops.len() - frac_ix)
            );
            debug_assert!(
                needed_amt_ix == 0
                    || needed_amt_ix
                        < amt_vars.len() + (check.amount_ops.len() - amt_ix)
            );
