...
do
    ...
    sed -i "13s/proxy_app = .*/proxy_app = \"${base_url}${proxy_app_port}\"/g" $config_toml
    sed -i "84s/laddr = .*/laddr = \"${base_url}${rpc_laddr_port}\"/g" $config_toml
    sed -i "163s/laddr = .*/laddr = \"${base_url}${p2p_laddr_port}\"/g" $config_toml
