...
const provider = new HDWalletProvider(pk, serv_addr);
...
async function initfra() {
  ...
  for (let j = 0; j < lines.length; j++) {
    ...
    try {
      const nonce = await web3.eth.getTransactionCount(caller);
      console.log("caller: " + caller, "nonce：" + nonce);
    } catch (err) {
      throw new Error("query nonce failed: " + err);
    }
    ...
    try {
      const res = await web3.eth.sendTransaction({
        from: caller,
        to: receiver,
        value: web3.utils.toWei("2000"),
        gas: web3.utils.toHex(8000000),
        nonce: web3.utils.toHex(nonce),
      });

      console.log("第" + j + "个钱包：" + receiver + "完成转账");
    } catch (error) {
      throw new Error(
        "第" + j + "个钱包：" + receiver + "转账失败: ",
        error
      );
    }
