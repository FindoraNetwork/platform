...
impl Env {
    ...
    fn alloc_resources(&mut self, id: NodeId, kind: Kind) -> Result<()> {
        ...
        cfg["p2p"]["seeds"] = toml_value(
            self.seed_nodes
                .values()
                .map(|n| format!("{}@127.0.0.1:{}", &n.tm_id, n.ports.tendermint_p2p))
                .collect::<Vec<_>>()
                .join(","),
        );

        // // Will be done in the function of `update_persistent_peers`
    ...
    fn update_persistent_peers(&self) -> Result<()> {
        for n in self
            .validator_nodes
            .values()
            .chain(self.full_nodes.values())
...
fn kill(pid: Pid) -> Result<()> {
    nixkill(NixPid::from_raw(pid), Signal::SIGTERM)
        .c(d!())
        .and_then(|_| nixkill(NixPid::from_raw(pid), Signal::SIGINT).c(d!()))
}
