...
use {
    ...
    noah::{
        anon_xfr::structs::OpenAnonAssetRecordBuilder, keys::SecretKey as XfrSecretKey,
    },
    ruc::*,
    std::{fmt, fs},
};
...
fn run() -> Result<()> {
    ...
    if matches.is_present("version") {
        ...
        let oabar = OpenAnonAssetRecordBuilder::from_abar(&abar, memo, &from)
            .unwrap()
            .build()
            .unwrap();
        ...
        //     None
        // };
        common::replace_staker(target, None, is_address_eth)?;
