...
pub fn stake_append(
    ...
    let kp = staker
        .c(d!())
        .and_then(|sk| wallet::restore_keypair_from_mnemonic_ed25519(sk).c(d!()))
...
pub fn show(basic: bool, is_address_eth: bool) -> Result<()> {
    ...
    let xfr_account = ruc::info!(get_keypair(is_address_eth)).map(|i| {
        println!(
...
pub fn gen_key(is_address_eth: bool) -> (String, String, String, XfrKeyPair) {
    let (mnemonic, key, kp) = loop {
        ...
        let kp: XfrKeyPair = if is_address_eth {
...
pub fn convert_bar2abar(
    ...
    let from = match owner_sk {
        Some(str) => ruc::info!(serde_json::from_str::<XfrSecretKey>(&format!("\"{str}\"")))
        .c(d!())?
        .into_keypair(),
...
/// # Arguments
/// * axfr_secret_key - XfrKeyPair in base64 form
/// * com             - Commitment in base64 form
/// * com_fra         - Commitment for paying fee
/// * amount          - amount to transfer
/// * to_axfr_public_key - XfrPublicKey in base64 form
...
pub fn get_owned_utxos(
    ...
    let list: Vec<(TxoSID, XfrAmount, XfrAssetType)> =
        utils::get_owned_utxos(&kp.get_pk())?
            .iter()
            .filter(|a| {
