...
pub fn stake_append(
    ...
    let kp = staker
        .c(d!())
        .and_then(|sk| {
            globutils::wallet::restore_keypair_from_mnemonic_default(sk).c(d!())
        })
...
pub fn show() -> Result<()> {
    ...
    let xfr_account = ruc::info!(get_keypair()).map(|i| {
        println!(
            ...
            globutils::wallet::public_key_to_bech32(&i.get_pk())
        );
        println!(
            "\x1b[31;01mFindora Public Key:\x1b[00m\n{}\n",
            globutils::wallet::public_key_to_base64(&i.get_pk())
        );
    });
...
pub fn transfer_fra(
    ...
    let from = owner_sk
        ...
        .or_else(|_| get_keypair().c(d!()))?;
    let to = globutils::wallet::public_key_from_base64(target_addr)
...
fn get_keypair() -> Result<XfrKeyPair> {
    if let Some(m_path) = MNEMONIC.as_ref() {
        fs::read_to_string(m_path)
            ...
            .and_then(|m| {
                globutils::wallet::restore_keypair_from_mnemonic_default(m.trim())
...
pub fn gen_key_and_print() {
    let (m, k, kp) = loop {
        let mnemonic = pnk!(globutils::wallet::generate_mnemonic_custom(24, "en"));
        let kp = pnk!(globutils::wallet::restore_keypair_from_mnemonic_default(
            &mnemonic
        ));
    ...
    let wallet_addr = globutils::wallet::public_key_to_bech32(kp.get_pk_ref());
    println!(
