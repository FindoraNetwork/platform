use evm_precompile_anemoi::Anemoi381;
...
fn main() {
    ...
    for _ in 0..1000 {
        _ = Anemoi381::execute_with_input_and_gas(&data[0..64], None);
    }
    let time = start.elapsed().as_nanos() / 1000;
    let gas = Anemoi381::GAS_PER_PERM;
    ...
    let start = Instant::now();
    for _ in 0..1000 {
        _ = Anemoi381::execute_with_input_and_gas(&data[0..1024], None);
    }
    let time = start.elapsed().as_nanos() / 1000;
    let gas = Anemoi381::GAS_PER_PERM * 11;
    println!("Result = {} ns every time", time);
    println!("Cost = {} every time", gas);
    println!("NS per gas = {}", (time as f64) / (gas as f64));
}
