...
fn read_fr(input: &[u8], start_inx: usize) -> Result<bn::Fr, PrecompileFailure> {
    ...
    bn::Fr::from_slice(&input[start_inx..(start_inx + 32)]).map_err(|_| {
...
fn read_point(input: &[u8], start_inx: usize) -> Result<bn::G1, PrecompileFailure> {
    ...
    let py =
        Fq::from_slice(&input[(start_inx + 32)..(start_inx + 64)]).map_err(|_| {
            PrecompileFailure::Error {
...
impl PrecompileId for Bn128Add {
    fn contract_id() -> u64 {
        0x06
    }
}
...
impl PrecompileId for Bn128Mul {
    fn contract_id() -> u64 {
        0x07
    }
}
...
impl PrecompileId for Bn128Pairing {
    fn contract_id() -> u64 {
        0x08
    }
}
