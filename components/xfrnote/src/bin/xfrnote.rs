...
pub fn make_xfr_note() -> XfrNote {
    ...
    let asset_type = AssetTypeCode::gen_random();
    let inputs_amounts = [
        (10u64, asset_type),
        (10u64, asset_type),
        (10u64, asset_type),
    ...
    for x in inputs_amounts.iter() {
        ...
        let asset_record = AssetRecordTemplate::with_no_asset_tracking(
            x.0,
            (x.1).val,
            asset_record_type,
            *keypair.get_pk_ref(),
        ...
        inputs.push(
            AssetRecord::from_template_no_identity_tracking(&mut prng, &asset_record)
                .unwrap(),
        );
    ...
    for x in outputs_amounts.iter() {
        ...
        let output =
            AssetRecord::from_template_no_identity_tracking(&mut prng, &ar).unwrap();
        outputs.push(output);
    }
