...
fn main() {
  ...
  let query_host = std::env::var_os("QUERY_SERVER_HOST").filter(|x| !x.is_empty())
                                                        .unwrap_or_else(|| "localhost".into());
  let ledger_host = std::env::var_os("LEDGER_HOST").filter(|x| !x.is_empty())
                                                   .unwrap_or_else(|| "localhost".into());
  let ledger_protocol = std::env::var_os("LEDGER_PROTOCOL").filter(|x| !x.is_empty())
