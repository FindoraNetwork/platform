...
use zei::xfr::structs::TracingPolicy;

fn inputs_error(msg: &str) -> Result<()> {
    Err(eg!(PlatformError::InputsError(Some(msg.to_owned()))))
}

fn io_error(msg: &str) -> Result<()> {
    Err(eg!(PlatformError::IoError(Some(msg.to_owned()))))
}
...
pub(crate) fn process_asset_issuer_cmd<T: RestfulLedgerAccess + RestfulLedgerUpdate>(
    ...
    match asset_issuer_matches.subcommand() {
        ...
        ("store_memos", Some(store_bar_and_memos_matches)) => {
            ...
            {
                let issuer_id = parse_to_u64(id_arg).c(d!())?;
                debug!("store_memos: got issuer_id={}", issuer_id);
                let issuer_pub_key =
                    data.get_asset_issuer_key_pair(issuer_id).c(d!())?.get_pk();
                debug!("store_memos: got issuer_pub_key={:?}", &issuer_pub_key);
                let tracer_enc_keys =
                    data.get_asset_tracer_key_pair(issuer_id).c(d!())?.enc_key;
            ...
            let record_type = AssetRecordType::from_flags(confidential_amount, false);
            ...
            {
                AssetTypeCode::new_from_base64(token_code).c(d!())?
            } else {
                eprintln!(
        ...
        ("define_asset", Some(define_asset_matches)) => {
            ...
            {
                let issuer_id = parse_to_u64(id_arg).c(d!())?;
                data.get_asset_issuer_key_pair(issuer_id).c(d!())?
            } else {
                eprintln!(
            ...
            if let Some(co_signer_ids) = define_asset_matches.value_of("cosigners") {
                ...
                {
                    weights = parse_to_u64_vec(cosignature_weights).c(d!())?;
                ...
                if let Some(thresh) = define_asset_matches.value_of("threshold") {
                    threshold = parse_to_u64(thresh).c(d!())?;
                } else {
            ...
            if let Some(token_code) = token_code {
                asset_token = AssetTypeCode::new_from_base64(token_code).c(d!())?;
                println!(
        ...
        ("set_kv", Some(kv_matches)) => {
            ...
            .c(d!())?;
            let key_pair = data.get_asset_issuer_key_pair(issuer_id).c(d!())?;
            let key = Key::hash(kv_matches.value_of("key").c(d!())?);
            ...
            txn_builder
                .add_operation_kv_update(&key_pair, &key, gen, Some(&hash))
                .c(d!())?;
        ...
        ("clear_kv", Some(kv_matches)) => {
            ...
            let key = Key::from_base64(kv_matches.value_of("key").c(d!())?)
                .c(d!(PlatformError::InputsError(None)))?;
            let key_pair = data.get_asset_issuer_key_pair(issuer_id).c(d!())?;
            let gen = parse_to_u64(
                kv_matches
            ...
            txn_builder
                .add_operation_kv_update(&key_pair, &key, gen, None)
                .c(d!())?;
            if let Some(txn_file) = txn_file {
                store_txn_to_file(&txn_file, &txn_builder)
            } else {
        ...
        ("issue_asset", Some(issue_asset_matches)) => {
            ...
            {
                AssetTypeCode::new_from_base64(token_code_arg).c(d!())?
            } else {
            ...
            if let Err(e) = txn_builder.add_basic_issue_asset(
                ...
                amount,
                AssetRecordType::from_flags(confidential_amount, false),
                &params,
            ) {
        ...
        ("transfer_asset", Some(transfer_asset_matches)) => {
            ...
            {
                parse_to_u64_vec(input_amounts_arg).c(d!())?
            } else {
                eprintln!(
            ...
            if let Some(recipients) = transfer_asset_matches.value_of("recipients") {
                let recipient_ids = parse_to_u64_vec(recipients).c(d!())?;
                for id in recipient_ids {
                    let recipient_pub_key =
                        data.get_borrower_key_pair(id).c(d!())?.get_pk();
            ...
            {
                parse_to_u64_vec(output_amounts_arg).c(d!())?
            } else {
                eprintln!(
        ...
        ("issue_and_transfer_asset", Some(issue_and_transfer_matches)) => {
            ...
            {
                let recipient_id = parse_to_u64(id_arg).c(d!())?;
                data.get_borrower_key_pair(recipient_id).c(d!())?
            } else {
                eprintln!(
        ...
        ("trace_and_verify_asset", Some(trace_and_verify_asset_matches)) => {
            ...
            {
                load_tracer_and_owner_memos_from_files(memo_file_arg).c(d!())?
            } else {
            ...
            {
                parse_to_u64(expected_amount_arg).c(d!())?
            } else {
                eprintln!(
        ...
        ("trace_credential", Some(trace_credential_matches)) => {
            ...
            {
                let issuer_id = parse_to_u64(id_arg).c(d!())?;
                let asset_tracer_key_pair =
                    data.get_asset_tracer_key_pair(issuer_id).c(d!())?;
            ...
            {
                load_tracer_memo_from_file(memo_file_arg).c(d!())?
            } else {
                eprintln!(
                ...
                trace_credential_matches.value_of("attribute")
            {
                let credential_issuer_public_key =
                    data.get_credential_issuer_key_pair(0).c(d!())?.0;
            ...
            match tracer_memo
                .verify_identity_attributes(&attrs_dec_key, &expected_value)
                .c(d!())
            {
                Ok(res) => {
                    if res[0] {
    ...
    match lender_matches.subcommand() {
        ...
        ("view_loan", Some(view_loan_matches)) => {
            ...
            if let Some(loan_arg) = view_loan_matches.value_of("loan") {
                let loan_id = parse_to_u64(loan_arg).c(d!())?;
...
>(
    ...
    match borrower_matches.subcommand() {
        ...
        ("request_loan", Some(request_loan_matches)) => {
            ...
            {
                parse_to_u64(interest_per_mille_arg).c(d!())?
            } else {
                eprintln!(
        ...
        (
            ...
            {
                parse_to_u64(credential_issuer_arg).c(d!())?
            } else {
                eprintln!(
        ...
        ("get_asset_record", Some(get_asset_record_matches)) => {
            ...
            let key_pair = data.get_borrower_key_pair(borrower_id).c(d!())?;
            let sid = if let Some(sid_arg) = get_asset_record_matches.value_of("sid") {
                TxoSID(parse_to_u64(sid_arg).c(d!())?)
            } else {
                eprintln!(
            ...
            let owner_memo = rest_client.get_owner_memo(sid.0).c(d!())?;
            let asset_record =
...
>(
    ...
    create_directory_and_rename_path(&expand_str, overwrite).c(d!())?;
    let seq_id = rest_client.get_block_commit_count().c(d!())?;
    ...
    if let Ok(txn_builder) = load_txn_from_file(txn_file) {
            ...
            || submit_matches.is_present("sids_file")
        {
            ...
            if let Some(path) = submit_matches.value_of("sids_file") {
                ...
                store_sids_to_file(path, &sids_str).c(d!())?;
            }
            Ok(())
        } else {
            rest_client
...
>(
    ...
    let dir = if let Some(dir) = inputs.value_of("dir") {
        ...
        let home_dir = if let Some(dir) = dirs::home_dir() {
            ...
            return Err(eg!(PlatformError::IoError(Some(
    ...
    match inputs.subcommand() {
        ...
        ("serialize", Some(_serialize_matches)) => {
            if let Some(txn_file) = inputs.value_of("txn") {
                let txn_builder = load_txn_from_file(&txn_file).c(d!(format!(
                    "File {} is not a valid transaction file.",
                    txn_file
                )))?;
...
pub fn get_cli_app<'a, 'b>() -> App<'a, 'b> {
    App::new("Transaction Builder")
        ...
        .arg(Arg::with_name("config").short("c").long("config").value_name("PATH/TO/FILE").help("Specify a custom config file (default: \"$FINDORA_DIR/config.toml\")").takes_value(true))
        .arg(Arg::with_name("local").long("local").help("If local flag is specified, transactions will be submitted to a local ledger"))
        .arg(Arg::with_name("dir").short("d").long("dir").value_name("PATH").help("Directory for configuaration, security, and temporary files; must be writable").takes_value(true).env("FINDORA_DIR"))
        .arg(Arg::with_name("txn").long("txn").value_name("FILE").help("Use named transaction file (will always be under findora_dir)").takes_value(true))
        .subcommand(
            SubCommand::with_name("asset_issuer")
                .subcommand(SubCommand::with_name("sign_up").arg(Arg::with_name("name").short("n").long("name").required(true).takes_value(true).help("Asset issuer's name.")))
                .arg(Arg::with_name("id").short("i").long("id").takes_value(true).help("Asset issuer id."))
                .subcommand(SubCommand::with_name("store_sids").arg(Arg::with_name("file").short("f").long("file").required(true).takes_value(true).help("Path to store the sids.")).arg(Arg::with_name("indices").short("i").long("indices").required(true).takes_value(true).help("Sids. Separate by comma (\",\").")))
                .subcommand(
                    SubCommand::with_name("store_memos")
                        .arg(Arg::with_name("file").short("f").long("file").required(true).takes_value(true).help("Path to store the tracer and owner memos."))
                        .arg(Arg::with_name("amount").short("a").long("amount").required(true).takes_value(true).help("Asset amount."))
                        .arg(Arg::with_name("confidential_amount").short("m").long("confidential_amount").takes_value(false).help("If specified, the amount will be confidential."))
                        .arg(Arg::with_name("token_code").short("t").long("token_code").required(true).takes_value(true).help("Asset token code.")),
                )
                .subcommand(
                    SubCommand::with_name("define_asset")
                        .arg(Arg::with_name("fiat").short("f").long("fiat").takes_value(false).help("Indicate the asset is a fiat asset."))
                        .arg(Arg::with_name("token_code").long("token_code").short("c").help("Explicit 16 character token code for the new asset; must be a unique name. If specified code is already in use, transaction will fail. If not specified, will display automatically generated token code.").takes_value(true))
                        .arg(Arg::with_name("traceable").short("trace").long("traceable").help("If specified, asset transfers can be traced by the issuer "))
                        .arg(Arg::with_name("max_units").takes_value(true).long("max_units").help("Set a cap on the total number of units of this asset that can be issued."))
                        .arg(Arg::with_name("non_transferable").long("non_transferable").help("Non-transferable assets can only be transferred by the asset issuer."))
                        .arg(Arg::with_name("updatable").long("updatable").help("Issuers of updatable assets can change their memos."))
                        .arg(Arg::with_name("cosigners").long("cosigners").takes_value(true).help("Ids of cosigners. Cosigners are additional signers needed for asset transfers."))
                        .arg(Arg::with_name("cosignature_weights").long("cosignature_weights").takes_value(true).help("Weights of each cosignature key, separated by comma. If unspecified, defaults to 1 for each cosigner."))
                        .arg(Arg::with_name("threshold").long("threshold").takes_value(true).help("Minimum sum of cosigner weights for a valid transfer. Default is the number of cosigners."))
                        .arg(Arg::with_name("memo").short("m").long("memo").required(true).takes_value(true).help("Memo as Json, with escaped quotation marks"))
                        .arg(Arg::with_name("confidential").short("xx").long("confidential").help("Make memo confidential"))
                        .arg(Arg::with_name("with_policy").short("p").help("TODO: add support for policies")),
                )
                .subcommand(
                    SubCommand::with_name("issue_asset")
                        .arg(Arg::with_name("token_code").short("c").long("token_code").required(true).takes_value(true).help("Token code of the asset to be issued. The transaction will fail if no asset with the token code exists."))
                        .arg(Arg::with_name("amount").short("amt").long("amount").required(true).takes_value(true).help("Amount of tokens to issue."))
                        .arg(Arg::with_name("confidential_amount").short("m").long("confidential_amount").takes_value(false).help("If specified, the amount will be confidential.")),
                )
                .subcommand(SubCommand::with_name("clear_kv").arg(Arg::with_name("key").short("k").long("key").required(true).takes_value(true).help("Which KV-store entry to clear")).arg(Arg::with_name("gen").short("g").long("gen").required(true).takes_value(true).help("Which generation of `key` this is")))
                .subcommand(
                    SubCommand::with_name("set_kv")
                        .arg(Arg::with_name("key").short("k").long("key").required(true).takes_value(true).help("Which KV-store entry to set. String passed in will be converted to a base-64 encoded key."))
                        .arg(Arg::with_name("gen").short("g").long("gen").required(true).takes_value(true).help("Which generation of `key` this is"))
                        .arg(Arg::with_name("value").short("v").long("value").required(true).takes_value(true).help("Data to commit to.")),
                )
                .subcommand(
                    SubCommand::with_name("transfer_asset")
                        .arg(Arg::with_name("recipients").short("r").long("recipients").required(true).takes_value(true).help("Recipients' ids. Separate by comma (\",\")."))
                        .arg(Arg::with_name("sids_file").short("s").long("sids_file").required(true).takes_value(true).help("Path to the input sids."))
                        .arg(Arg::with_name("issuance_txn_files").short("f").long("issuance_txn_files").required(true).takes_value(true).help("Paths to the asset issuance transactions."))
                        .arg(Arg::with_name("traceable").short("t").long("traceable").help("If specified, the asset will be traceable."))
                        .arg(Arg::with_name("input_amounts").short("iamts").long("input_amounts").required(true).takes_value(true).help("Amount to transfer from each record. Separate by comma (\",\")."))
                        .arg(Arg::with_name("output_amounts").short("oamts").long("output_amounts").required(true).takes_value(true).help("Amount to transfer to each account. Separate by comma (\",\").")),
                )
                .subcommand(
                    SubCommand::with_name("issue_and_transfer_asset")
                        .arg(Arg::with_name("recipient").short("r").long("recipient").required(true).takes_value(true).help("Recipient's id."))
                        .arg(Arg::with_name("amount").short("amt").long("amount").required(true).takes_value(true).help("Amount of tokens to issue and transfer."))
                        .arg(Arg::with_name("token_code").short("tc").long("token_code").required(true).takes_value(true).help("Token code of the asset."))
                        .arg(Arg::with_name("memo_file").short("f").long("memo_file").takes_value(true).help("If specified, will store the asset tracer memo and owner memo."))
                        .arg(Arg::with_name("confidential_amount").short("m").long("confidential_amount").takes_value(false).help("If specified, the amount will be confidential."))
                        .arg(Arg::with_name("confidential_asset").short("s").long("confidential_asset").takes_value(false).help("If specified, the asset transfer will be confidential.")),
                )
                .subcommand(SubCommand::with_name("trace_and_verify_asset").arg(Arg::with_name("memo_file").short("f").long("memo_file").required(true).takes_value(true).help("Path to the tracer and owner memos.")).arg(Arg::with_name("expected_amount").short("a").long("expected_amount").required(true).takes_value(true).help("Expected asset amount to verify.")))
                .subcommand(
                    SubCommand::with_name("trace_credential")
                        .arg(Arg::with_name("memo_file").short("f").long("memo_file").required(true).takes_value(true).help("Path to the tracer and owner memos."))
                        .arg(Arg::with_name("attribute").short("a").long("attribute").required(true).takes_value(true).possible_values(&["min_credit_score", "min_income", "citizenship"]).help("Credential attribute to verify."))
                        .arg(Arg::with_name("expected_value").short("v").long("expected_value").required(true).takes_value(true).help("Expected credential value to verify.")),
                ),
        )
        .subcommand(SubCommand::with_name("credential_issuer").subcommand(SubCommand::with_name("sign_up").arg(Arg::with_name("name").short("n").long("name").required(true).takes_value(true).help("Credential issuer's name."))).arg(Arg::with_name("id").short("i").long("id").takes_value(true).help("Credential issuer id.")))
        .subcommand(
            SubCommand::with_name("lender")
                .subcommand(SubCommand::with_name("sign_up").arg(Arg::with_name("name").short("n").long("name").required(true).takes_value(true).help("Lender's name.")))
                .arg(Arg::with_name("id").short("i").long("id").takes_value(true).help("Lender id."))
                .subcommand(
                    SubCommand::with_name("view_loan")
                        .arg(Arg::with_name("loan").short("l").long("loan").takes_value(true).help("Display the loan with the specified id only."))
                        .arg(Arg::with_name("filter").short("f").long("filter").takes_value(true).possible_values(&["requested", "fulfilled", "declined", "active", "complete"]).help("Display the loan with the specified status only."))
                        .help("By default, display all loans of this lender."),
                )
                .subcommand(
                    SubCommand::with_name("fulfill_loan")
                        .arg(Arg::with_name("loan").short("l").long("loan").required(true).takes_value(true).help("Loan id."))
                        .arg(Arg::with_name("issuer").short("i").long("issuer").required(true).takes_value(true).help("Asset issuer id."))
                        .arg(Arg::with_name("memo_file").short("f").long("memo_file").takes_value(true).help("If specified, will store the asset tracer memo and owner memo."))
                        .arg(Arg::with_name("http").long("http").takes_value(false).help("Specify that http, not https should be used."))
                        .arg(Arg::with_name("localhost").long("localhost").takes_value(false).help("Specify that localhost, not testnet.findora.org should be used.")),
                )
                .subcommand(
                    SubCommand::with_name("create_or_overwrite_requirement")
                        .arg(Arg::with_name("attribute").short("a").long("attribute").required(true).takes_value(true).possible_values(&["min_credit_score", "min_income", "citizenship"]).help("Credential attribute."))
                        .arg(Arg::with_name("requirement").short("r").long("requirement").required(true).takes_value(true).help("Required value of the credential record."))
                        .help("Create or overwrite a credential requirement."),
                ),
        )
        .subcommand(
            SubCommand::with_name("borrower")
                .subcommand(SubCommand::with_name("sign_up").arg(Arg::with_name("name").short("n").long("name").required(true).takes_value(true).help("Borrower's name.")))
                .arg(Arg::with_name("id").short("i").long("id").takes_value(true).help("Borrower id."))
                .subcommand(
                    SubCommand::with_name("load_funds")
                        .arg(Arg::with_name("issuer").short("i").long("issuer").takes_value(true).help("Required: issuer id."))
                        .arg(Arg::with_name("amount").short("a").long("amount").required(true).takes_value(true).help("Amount to transfer to the recipient."))
                        .arg(Arg::with_name("http").long("http").takes_value(false).help("Specify that http, not https should be used."))
                        .arg(Arg::with_name("localhost").long("localhost").takes_value(false).help("Specify that localhost, not testnet.findora.org should be used.")),
                )
                .subcommand(
                ...
                .subcommand(
                    SubCommand::with_name("request_loan")
                        .arg(Arg::with_name("lender").short("l").long("lender").required(true).takes_value(true).help("Lender id."))
                        .arg(Arg::with_name("amount").short("a").long("amount").required(true).takes_value(true).help("Amount of the loan."))
                        .arg(Arg::with_name("interest_per_mille").short("i").long("interest_per_mille").required(true).takes_value(true).help("Interest per mille. The interest rate will be interest_per_mille/1000."))
                        .arg(Arg::with_name("duration").short("d").long("duration").required(true).takes_value(true).help("Payment duration")),
                )
                .subcommand(
                    SubCommand::with_name("pay_loan")
                        .arg(Arg::with_name("loan").short("l").long("loan").required(true).takes_value(true).help("Loan id."))
                        .arg(Arg::with_name("amount").short("a").long("amount").required(true).takes_value(true).help("Payment amount."))
                        .arg(Arg::with_name("http").long("http").takes_value(false).help("Specify that http, not https should be used."))
                        .arg(Arg::with_name("localhost").long("localhost").takes_value(false).help("Specify that localhost, not testnet.findora.org should be used.")),
                )
                .subcommand(SubCommand::with_name("view_credential").arg(Arg::with_name("attribute").short("a").long("attribute").takes_value(true).possible_values(&["min_credit_score", "min_income", "citizenship"]).help("Display the specified credential attribute only.")).help("By default, display all credentials of this borrower."))
                .subcommand(
                    SubCommand::with_name("create_or_overwrite_credential")
                        .arg(Arg::with_name("credential_issuer").short("c").long("credential_issuer").required(true).takes_value(true).help("Credential issuer id."))
                        .arg(Arg::with_name("attribute").short("a").long("attribute").required(true).takes_value(true).possible_values(&["min_credit_score", "min_income", "citizenship"]).help("Credential attribute."))
                        .arg(Arg::with_name("value").short("v").long("value").required(true).takes_value(true).help("Value of the credential record."))
                        .help("Create or overwrite a credential record."),
                )
                .subcommand(
                    SubCommand::with_name("get_asset_record")
                        .arg(Arg::with_name("sid").long("sid").short("s").takes_value(true).help("Asset sid."))
                        .arg(Arg::with_name("memo_file").short("f").long("memo_file").takes_value(true).help("Path to the tracer and owner memos."))
                        .arg(Arg::with_name("http").long("http").takes_value(false).help("Specify that http, not https should be used."))
                        .arg(Arg::with_name("localhost").long("localhost").takes_value(false).help("Specify that localhost, not testnet.findora.org should be used.")),
                ),
        )
        .subcommand(
            SubCommand::with_name("create_txn_builder")
                .about("By default, will rename previous file with a .<number> suffix")
                .arg(Arg::with_name("name").short("n").long("name").value_name("FILE").help("Specify a name for newly created transaction file").takes_value(true))
                .arg(Arg::with_name("overwrite").long("force").alias("overwrite").short("f").help("If specified, the existing file with the same name will be overwritten.")),
        )
        .subcommand(SubCommand::with_name("serialize"))
        .subcommand(
            SubCommand::with_name("submit")
                .arg(Arg::with_name("get_sids").long("get_sids").short("g").takes_value(false).help("If specified, will query the utxo sids."))
                .arg(Arg::with_name("sids_file").long("sids_file").short("s").takes_value(true).help("If specified, will store the utxo sids to the file."))
                .arg(Arg::with_name("http").long("http").takes_value(false).help("Specify that http, not https should be used."))
                .arg(Arg::with_name("localhost").long("localhost").takes_value(false).help("Specify that localhost, not testnet.findora.org should be used.")),
        )
}
