...
use zei::xfr::structs::{
    AssetRecordTemplate, AssetTracerKeyPair, BlindAssetRecord, OpenAssetRecord,
    OwnerMemo, TracerMemo, TracingPolicies, TracingPolicy,
};
...
fn INIT_DATA_PATH() -> String {
    ...
    ret.push(
        std::env::var_os("FINDORA_TXN_CLI_DATA_SEARCH_PATH")
            .filter(|x| !x.is_empty())
            .unwrap_or_else(|| ".".into()),
    );
...
pub(crate) type TracerAndOwnerMemos = (Vec<TracerMemo>, Option<OwnerMemo>);
...
impl CredentialIndex {
    ...
    pub(crate) fn get_credential_index(index: u64) -> Result<Self> {
        match index {
...
impl AssetIssuer {
    pub fn new(id: usize, name: String) -> Result<Self> {
        ...
        let tracer_key_pair =
            AssetTracerKeyPair::generate(&mut ChaChaRng::from_entropy());
        let tracer_key_pair_str =
            serde_json::to_string(&tracer_key_pair).c(d!(ser_fail!()))?;
...
impl CredentialIssuer {
    ...
    pub fn new(
        ...
        let key_pair = credential_issuer_key_gen(
            &mut ChaChaRng::from_entropy(),
            &attribute_names_and_sizes,
        );
        let key_pair_str = serde_json::to_string(&key_pair).c(d!(ser_fail!()))?;
...
impl Data {
    ...
    pub fn add_asset_issuer(&mut self, data_dir: &str, name: String) -> Result<()> {
        let id = self.asset_issuers.len();
        self.asset_issuers
            .push(AssetIssuer::new(id, name.clone()).c(d!())?);
        println!("{}'s id is {}.", name, id);
        store_data_to_file(self.clone(), data_dir)
    }

    pub fn get_asset_issuer_key_pair(&self, id: u64) -> Result<XfrKeyPair> {
        let key_pair_str = &self.asset_issuers[id as usize].key_pair;
        XfrKeyPair::zei_from_bytes(&hex::decode(key_pair_str).c(d!(ser_fail!()))?)
            .c(d!(PlatformError::ZeiError(None)))
    }

    pub fn get_asset_tracer_key_pair(&self, id: u64) -> Result<AssetTracerKeyPair> {
        let tracer_key_pair_str = &self.asset_issuers[id as usize].tracer_key_pair;
        let tracer_key_pair =
            serde_json::from_str(&tracer_key_pair_str).c(d!(des_fail!()))?;
        Ok(tracer_key_pair)
    }

    pub fn add_credential_issuer(
        &mut self,
        data_dir: &str,
        name: String,
        attributes: Vec<CredentialIndex>,
        ...
        self.credential_issuers
            .push(CredentialIssuer::new(id, name.clone(), attributes).c(d!())?);
        println!("{}'s id is {}.", name, id);
        store_data_to_file(self.clone(), data_dir)
    }
    ...
    pub fn get_credential_issuer_key_pair(
        &self,
        id: u64,
    ) -> Result<(CredIssuerPublicKey, CredIssuerSecretKey)> {
        let key_pair_str = &self.credential_issuers[id as usize].key_pair;
        let key_pair = serde_json::from_str(&key_pair_str).c(d!(des_fail!()))?;
        Ok(key_pair)
    }

    pub fn add_lender(&mut self, data_dir: &str, name: String) -> Result<()> {
    ...
    pub(crate) fn get_lender_key_pair(&self, id: u64) -> Result<XfrKeyPair> {
        let key_pair_str = &self.lenders[id as usize].key_pair;
        XfrKeyPair::zei_from_bytes(&hex::decode(key_pair_str).c(d!(des_fail!()))?)
            .c(d!(PlatformError::ZeiError(None)))
    }
    ...
    pub fn create_or_overwrite_requirement(
        &mut self,
        data_dir: &str,
        lender_id: u64,
    ...
    pub fn add_borrower(&mut self, data_dir: &str, name: String) -> Result<()> {
    ...
    pub fn get_borrower_key_pair(&self, id: u64) -> Result<XfrKeyPair> {
    ...
    pub fn create_or_overwrite_credential(
        &mut self,
        data_dir: &str,
        borrower_id: u64,
        ...
        if let Some(credential_id) = self.borrowers[borrower_id as usize].credentials {
            if self.credentials[credential_id as usize].values[attribute as usize]
                .clone()
                == None
                && credential_issuer_id
                    == self.credentials[credential_id as usize].credential_issuer
            {
            ...
            self.credentials.push(Credential::new(
                credential_id as u64,
                borrower_id,
                credential_issuer_id,
                values,
            ));
            self.borrowers[borrower_id as usize].credentials =
...
pub fn get_and_update_sequence_number(data_dir: &str) -> Result<u64> {
...
pub fn parse_to_u64(val_str: &str) -> Result<u64> {
...
pub fn parse_to_u64_vec(vals_str: &str) -> Result<Vec<u64>> {
...
pub fn load_data(data_dir: &str) -> Result<Data> {
    let data_file_path = format!("{}/{}", data_dir, DATA_FILE);
    match fs::read_to_string(&data_file_path) {
        Ok(data) => serde_json::from_str::<Data>(&data).c(d!(des_fail!())),
        Err(e1) => match fs::read_to_string(&INIT_DATA_PATH()) {
            Ok(init_data) => {
                let data =
                    serde_json::from_str::<Data>(&init_data).c(d!(des_fail!()))?;
                store_data_to_file(data.clone(), data_dir).c(d!())?;
                Ok(data)
            }
            Err(e2) => Err(eg!(PlatformError::IoError(Some(format!(
...
/// # Arguments
/// * `file_path`: file path.
pub fn load_txn_from_file(file_path: &str) -> Result<TransactionBuilder> {
    let txn = fs::read_to_string(file_path).c(d!(PlatformError::IoError(Some(
        format!("Failed to read file: {}", file_path)
    ))))?;
    debug!("Parsing builder from file contents: \"{}\"", &txn);
    match serde_json::from_str(&txn) {
        Ok(builder) => Ok(builder),
        Err(e) => Err(eg!(des_fail!(e))),
    }
}
...
/// # Arguments
/// * `file_path`: file path
pub fn load_sids_from_file(file_path: &str) -> Result<Vec<u64>> {
    let sids_str = fs::read_to_string(file_path).c(d!(PlatformError::IoError(Some(
        format!("Failed to read file: {}", file_path)
    ))))?;
...
pub(crate) fn load_blind_asset_record_and_owner_memo_from_file(
    file_path: &str,
) -> Result<(BlindAssetRecord, Option<OwnerMemo>)> {
    let txn = fs::read_to_string(file_path).c(d!(PlatformError::IoError(Some(
        format!("Failed to read file: {}", file_path)
    ))))?;
    ...
    match serde_json::from_str::<TransactionBuilder>(&txn) {
        Ok(builder) => Ok((
            builder.get_output_ref(0).record,
            builder.get_owner_memo_ref(0).cloned(),
        )),
        Err(e) => Err(eg!(des_fail!(e))),
    }
}
...
pub(crate) fn load_open_asset_record_from_file(
    file_path: &str,
    key_pair: &XfrKeyPair,
...
pub fn load_tracer_memo_from_file(file_path: &str) -> Result<TracerMemo> {
    let tracer_memo = fs::read_to_string(file_path).c(d!(PlatformError::IoError(
        Some(format!("Failed to read file: {}", file_path))
    )))?;
    debug!(
        "Parsing tracer memo from file contents: \"{}\"",
        &tracer_memo
    );
    serde_json::from_str::<TracerMemo>(&tracer_memo).c(d!(des_fail!()))
}
...
pub fn load_tracer_and_owner_memos_from_files(
    file_paths: &str,
) -> Result<Vec<TracerAndOwnerMemos>> {
    let mut tracer_and_owner_memos = Vec::new();
    for file_path in split_arg(file_paths) {
        let memos = fs::read_to_string(file_path).c(d!(PlatformError::IoError(
            Some(format!("Failed to read file: {}", file_path))
        )))?;
        debug!(
            "Parsing tracer and owner memos from file contents: \"{}\"",
            &memos
        );
...
/// # Arguments
/// * `data`: data to store.
pub(crate) fn store_data_to_file(data: Data, data_dir: &str) -> Result<()> {
    let data_file_path = format!("{}/{}", data_dir, DATA_FILE);
    if let Ok(as_json) = serde_json::to_string(&data) {
        if let Ok(()) = fs::create_dir_all(&data_dir) {
            if let Err(error) = fs::write(data_file_path, &as_json) {
                return Err(eg!(PlatformError::IoError(Some(format!(
...
pub fn store_txn_to_file(path_str: &str, txn: &TransactionBuilder) -> Result<()> {
    if let Ok(as_json) = serde_json::to_string(txn) {
        ...
        Err(eg!(PlatformError::IoError(Some(format!(
...
pub fn store_sids_to_file(path_str: &str, sids: &str) -> Result<()> {
...
pub(crate) fn store_tracer_memo_to_file(
    path_str: &str,
    tracer_memo: TracerMemo,
) -> Result<()> {
...
pub fn store_tracer_and_owner_memos_to_file(
    path_str: &str,
    tracer_and_owner_memos: TracerAndOwnerMemos,
) -> Result<()> {
    debug!(
        "store_tracer_and_owner_memos_to_file: path_str = {}, tracer_and_owner_memos = {:?}",
...
pub fn get_and_store_memos_to_file(
    path_str: &str,
    pub_key: XfrPublicKey,
    amount: u64,
    token_code: AssetTypeCode,
    record_type: AssetRecordType,
    policy: Option<TracingPolicy>,
) -> Result<()> {
    let (_, tracer_memo, owner_memo) = get_blind_asset_record_and_memos(
        pub_key,
        amount,
        token_code,
        record_type,
    ...
    .c(d!())?;
    store_tracer_and_owner_memos_to_file(path_str, (tracer_memo, owner_memo)).c(d!())
}
...
pub(crate) fn create_directory_if_missing(path_str: &str) -> Result<()> {
    ...
    if let Some(parent) = path.parent() {
        ...
        if let Err(error) = fs::create_dir_all(&parent) {
            return Err(eg!(PlatformError::IoError(Some(format!(
                "Failed to create directory for the parent path of {}: {}",
                path_str, error
            )))));
        }
    }
...
pub fn create_directory_and_rename_path(path_str: &str, overwrite: bool) -> Result<()> {
    let path = Path::new(&path_str);
    create_directory_if_missing(&path_str).c(d!())?;
    if path.exists() && !overwrite {
        rename_existing_path(&path).c(d!())?;
    }
    Ok(())
}
...
pub(crate) fn find_available_path(path: &Path, n: i32) -> Result<PathBuf> {
    if n < BACKUP_COUNT_MAX {
        ...
        Err(eg!(PlatformError::IoError(Some(format!(
            "Too many backups for {:?}. Use --path to specify another path.",
            path
        )))))
    }
}
...
pub(crate) fn next_path(path: &Path) -> Result<PathBuf> {
    pub(crate) fn add_backup_extension(path: &Path) -> Result<PathBuf> {
        ...
        if let Some(name) = path.file_name() {
            if let Some(name_str) = name.to_str() {
                ...
                Err(eg!(PlatformError::IoError(Some(
                    "Failed to convert the path to string.".to_owned(),
                ))))
            }
        } else {
            Err(eg!(PlatformError::IoError(Some(
                "Failed to get the file name.".to_owned(),
            ))))
        }
    }
    ...
    if let Some(ext) = path.extension() {
        ...
        if path.components().next() == None {
            ...
            find_available_path(&add_backup_extension(&path).c(d!())?, 0)
        }
    }
}
...
/// # Arguments
/// * `path`: file path.
pub fn rename_existing_path(path: &Path) -> Result<()> {
    let next = next_path(path).c(d!())?;
