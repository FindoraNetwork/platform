...
use credentials::{
    ...
    credential_sign, credential_user_key_gen, credential_verify, CredCommitment,
    CredCommitmentKey, CredIssuerPublicKey, CredIssuerSecretKey, CredPoK, CredSignature,
    CredUserPublicKey, CredUserSecretKey, Credential,
};
...
const HELP_SHOWCREDS_STRING: &str =
...
fn add_issuer(
    ...
    fn new_issuer(global_state: &mut GlobalState, issuer_name: &str) -> Issuer {
        ...
        let (issuer_pk, issuer_sk) =
            credential_issuer_key_gen(&mut global_state.prng, &attrs);
        Issuer {
...
fn add_user(
    ...
    if global_state.users.get(user_name).is_some() {
        ...
        if let Some(issuer) = global_state.issuers.get(issuer_name) {
            let (user_pk, user_sk) =
                credential_user_key_gen(&mut global_state.prng, &issuer.public_key);
            let user = User {
...
fn issue_credential(
    ...
    match (
        global_state.issuers.get(issuer),
        global_state.users.get(user),
    ) {
        (Some(issuer_keys), Some(user_keys)) => {
            let attr_as_slice: Vec<(String, &[u8])> = attrs
                .iter()
                .map(|(k, v)| (k.clone(), v.as_bytes()))
                .collect();
            let sig = credential_sign(
                &mut global_state.prng,
...
fn user_commit(global_state: &mut GlobalState, user: &str) -> Result<(), String> {
    match (
        ...
        (Some(user_struct), Some(sig)) => {
            ...
            if let Ok((commitment, _proof, key)) = credential_commit(
                &mut global_state.prng,
...
fn user_selectively_reveal(
    ...
    match (
        ...
        (Some(user_struct), Some((_commitment, key)), Some(sig)) => {
            ...
            let map: Vec<(String, Vec<u8>)> = attrs
                .iter()
                .map(|(k, v)| (k.clone(), v.as_bytes().to_vec()))
                .collect();
            let credential = Credential {
            ...
            if let Ok(pok) = credential_open_commitment(
                &mut global_state.prng,
                &user_struct.secret_key,
                &credential,
                &key,
        ...
        (None, None, None) => {
            Err("Unable to find either user or commitment".to_string())
        }
...
fn verify(
    ...
    let (commitment_opt, merkle_proof) =
        global_state.smt.get_with_proof(address.clone());
    if let Some(commitment_string) = commitment_opt {
        ...
        let attrs_vec: Vec<(String, &[u8])> = attrs
            .iter()
            .map(|(k, v)| (k.clone(), v.as_bytes()))
            .collect();
        if global_state
            ...
            .check_merkle_proof(&address, commitment_opt, &merkle_proof)
        {
            ...
            if let Err(e) = credential_verify(
                &user_info.issuer_pk,
                &attrs_vec[..],
                &commitment,
                &pok,
                ...
                Err(format!(
                    "smt merkle proof succeeds but ac_verify fails with {}",
                    e
                ))
            } else {
...
fn main() -> Result<(), rustyline::error::ReadlineError> {
    ...
    let args = parse_args();
    let _registry_path =
        Path::new(args.value_of("registry").unwrap_or(DEFAULT_REGISTRY_PATH));
...
mod tests {
    ...
    use credentials::{
        credential_commit, credential_issuer_key_gen, credential_open_commitment,
        credential_sign, credential_user_key_gen, credential_verify, Credential,
    };
    use rand_chacha::ChaChaRng;
    use rand_core::SeedableRng;
    ...
    fn test_ac_functions() {
        ...
        let (issuer_pk, issuer_sk) =
            credential_issuer_key_gen::<ChaChaRng>(&mut prng, &attrs_temp);
        let (user_pk, user_sk) =
            credential_user_key_gen::<ChaChaRng>(&mut prng, &issuer_pk);
        ...
        let signature =
            credential_sign(&mut prng, &issuer_sk, &user_pk, &attributes).unwrap(); // Done by Issuer

        // Enter the User
        let attr_map = vec![
            (dob.clone(), dob_attr.to_vec()),
            (ss.clone(), ss_attr.to_vec()),
            (dl, dl_attr.to_vec()),
        ];
        let credential = Credential {
            signature,
        ...
        let reveal_pok =
            credential_open_commitment(&mut prng, &user_sk, &credential, &key, &bitmap)
                .unwrap();

        // Enter the Verifier with user generated POK (and address in the AIR tests)
        let attr_map = [(dob, &dob_attr[..])];
