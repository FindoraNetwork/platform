...
pub fn verify_authenticated_txn(
    ...
    let state_commitment = serde_json::from_str::<BitDigest>(&state_commitment)
...
pub fn create_blind_asset_record(
    amount: u64,
    code: String,
    pk: &XfrPublicKey,
    conf_amount: bool,
...
impl WasmTransactionBuilder {
    ...
    pub fn add_basic_issue_asset(
        ...
        if elgamal_pub_key.is_empty() {
            ...
            issuer_keys = Some(AssetIssuerPubKeys {
                eg_ristretto_pub_key: pk,
    ...
    pub fn add_operation_issue_asset(
        ...
        let asset_token = AssetTypeCode::new_from_base64(&code)
            .map_err(|_e| JsValue::from_str("Could not deserialize asset token code"))?;
        let blind_asset_record = serde_json::from_str::<BlindAssetRecord>(&record)
            .map_err(|_e| JsValue::from_str("could not deserialize blind asset record"))?;
    ...
    pub fn add_operation(&mut self, op: String) -> Result<WasmTransactionBuilder, JsValue> {
        let op = serde_json::from_str::<Operation>(&op)
            .map_err(|_e| JsValue::from_str("Could not deserialize operation"))?;
        Ok(WasmTransactionBuilder {
            transaction_builder: Serialized::new(
                &*self.transaction_builder.deserialize().add_operation(op),
            ),
        })
    }
...
impl WasmTransferOperationBuilder {
    ...
    pub fn add_input(
        ...
        let txo_sid = serde_json::from_str::<TxoRef>(&txo_ref)
            .map_err(|_e| JsValue::from_str("Could not deserialize txo sid"))?;
    ...
    pub fn add_output(
        ...
        let new_builder = Serialized::new(
            &*self
                .op_builder
                .deserialize()
