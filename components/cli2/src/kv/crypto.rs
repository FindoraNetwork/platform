...
impl Key {
    ...
    pub fn from_password(password: &[u8], salt: &[u8]) -> Key {
        ...
        let hash = Zeroizing::new(
            argon2::hash_raw(password, salt, &config).expect("Failure Hashing Password"),
        );
...
where
    Clear: Serialize + DeserializeOwned + 'static,
    Encrypted: Serialize + DeserializeOwned + 'static,
{
    ...
    pub fn pack(clear: Clear, encrypted: &Encrypted, password: &[u8]) -> Self {
        ...
        let encrypted =
            serde_json::to_string(encrypted).expect("JSON serialization failed");
        ...
        let encrypted = cipher
            .encrypt(Nonce::from_slice(&nonce[..]), encrypted.as_bytes())
