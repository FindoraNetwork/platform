...
pub enum KVError {
    ...
    #[snafu(display(
        "Attempted to call KVStore::with on a key that doesn't exist: {}",
        key
    ))]
...
impl KVStore {
    ...
    pub fn open(db_path: impl AsRef<Path>) -> Result<KVStore> {
        ...
        let conn = Connection::open(db_path).with_context(|| Open {
    ...
    pub fn open_in_memory() -> Result<KVStore> {
        let conn = Connection::open_in_memory().with_context(|| Open {
    ...
    fn table_exists<T: HasTable>(&self) -> Result<bool> {
        ...
        let mut stmt = self.db.prepare(&name_query).with_context(|| Prepare {
            statement: name_query.to_string(),
        })?;
    ...
    pub fn get<T: HasTable>(&self, id: &T::Key) -> Result<Option<T>> {
        ...
        let mut stmt = self.db.prepare(&get_query).context(Prepare {
            statement: get_query,
        })?;
        let rows = stmt
        ...
        let data = serde_json::from_str(&data_json).context(Deserialization {
            table,
            json: data_json,
        })?;
        Ok(Some(data))
    ...
    pub fn set<T: HasTable>(&self, key: &T::Key, value: T) -> Result<Option<T>> {
        ...
        if old_value.is_some() {
            ...
            let mut stmt = self.db.prepare(&update_query).context(Prepare {
            ...
            let mut stmt = self.db.prepare(&set_query).context(Prepare {
                statement: set_query,
            })?;
    ...
    pub fn get_all<T: HasTable>(&self) -> Result<BTreeMap<T::Key, T>> {
        ...
        let mut stmt = self.db.prepare(&get_all_query).context(Prepare {
            statement: get_all_query,
        })?;
        let rows = stmt
        ...
        for (key, value) in rows {
            let key = serde_json::from_str(&key).with_context(|| Deserialization {
                table: T::TABLE_NAME.to_string(),
                json: key,
            })?;
            let value =
                serde_json::from_str(&value).with_context(|| Deserialization {
                    table: T::TABLE_NAME.to_string(),
                    json: value,
                })?;
    ...
    pub fn delete<T: HasTable>(&self, key: &T::Key) -> Result<Option<T>> {
        ...
        let mut stmt = self.db.prepare(&delete_query).context(Prepare {
...
impl CliDataStore for KVStore {
    ...
    >(
        ...
        let keypair = crate::helpers::prompt_with_retries(3, Some(&k.0), |password| {
            let mixed_pair = self.get_encrypted_raw::<XfrKeyPair>(k).map_err(|_| {
            ...
            let mixed_pair =
                mixed_pair.with_context(|| WithInvalidKey { key: k.0.clone() })?;
    ...
    fn get_pubkeys(
        &self,
    ) -> Result<BTreeMap<crate::PubkeyName, XfrPublicKey>, CliError> {
        Ok(self.get_all()?)
    }
    ...
    fn get_txn_builders(
        &self,
    ) -> Result<BTreeMap<crate::TxnBuilderName, TxnBuilderEntry>, CliError> {
        Ok(self.get_all()?)
    }
    fn get_txn_builder(
        &self,
    ...
    fn get_cached_txos(
        &self,
    ) -> Result<BTreeMap<crate::TxoName, crate::TxoCacheEntry>, CliError> {
        Ok(self.get_all()?)
    }
    fn get_cached_txo(
        &self,
        k: &crate::TxoName,
    ) -> Result<Option<crate::TxoCacheEntry>, CliError> {
        Ok(self.get(k)?)
    }
