...
fn test_create_users() {
    ...
    let output = sign_up_asset_issuer(dir, "Issuer AI")
        .expect("Failed to create an asset issuer");
    ...
    let output = sign_up_credential_issuer(dir, "Issuer CI")
        .expect("Failed to create a credential issuer");
    io::stdout().write_all(&output.stdout).unwrap();
    io::stdout().write_all(&output.stderr).unwrap();
...
fn test_define_asset_with_help() {
    let output = Command::new(&COMMAND())
        .args(&["asset_issuer", "define_asset", "--help"])
        .output()
        .expect("failed to execute process");
...
fn test_issue_asset_with_help() {
    let output = Command::new(&COMMAND())
        .args(&["asset_issuer", "issue_asset", "--help"])
        .output()
        .expect("failed to execute process");
...
fn test_transfer_asset_with_help() {
    let output = Command::new(&COMMAND())
        .args(&["asset_issuer", "transfer_asset", "--help"])
        .output()
        .expect("failed to execute process");
...
fn test_invalid_valid_overwrite_and_rename_path() {
    ...
    let output =
        create_txn_builder_overwrite_path(path).expect("Failed to execute process");
...
fn test_create_txn_builder_with_name() {
    // Create transaction builder
    let output = create_txn_builder_with_path("txn_builder")
