...
impl AssetIssuer {
    pub fn new(id: usize, name: String) -> Result<Self, PlatformError> {
        ...
        let tracer_key_pair_str =
            serde_json::to_string(&tracer_key_pair).map_err(|e| ser_fail!(e))?;
...
impl Data {
    ...
    pub fn get_asset_issuer_key_pair(
        &self,
        id: u64,
    ) -> Result<XfrKeyPair, PlatformError> {
    ...
    pub fn add_credential_issuer(
        ...
        self.credential_issuers.push(CredentialIssuer::new(
            id,
            name.clone(),
            attributes,
        )?);
    ...
    pub(crate) fn get_lender_key_pair(
        &self,
        id: u64,
    ) -> Result<XfrKeyPair, PlatformError> {
...
pub fn load_data(data_dir: &str) -> Result<Data, PlatformError> {
    ...
    match fs::read_to_string(&data_file_path) {
        ...
        Err(e1) => match fs::read_to_string(&INIT_DATA_PATH()) {
            Ok(init_data) => {
                let data = serde_json::from_str::<Data>(&init_data)
                    .map_err(|e| des_fail!(e))?;
                store_data_to_file(data.clone(), data_dir)?;
                Ok(data)
            }
            Err(e2) => Err(PlatformError::IoError(format!(
...
pub(crate) fn load_open_asset_record_from_file(
    ...
    let (blind_asset_record, owner_memo) =
        load_blind_asset_record_and_owner_memo_from_file(file_path)?;
    open_blind_asset_record(&blind_asset_record, &owner_memo, key_pair.get_sk_ref())
...
pub fn load_tracer_memo_from_file(
    file_path: &str,
...
/// # Arguments
/// * `data`: data to store.
pub(crate) fn store_data_to_file(
    data: Data,
    data_dir: &str,
) -> Result<(), PlatformError> {
    let data_file_path = format!("{}/{}", data_dir, DATA_FILE);
...
pub fn store_txn_to_file(
    ...
    if let Ok(as_json) = serde_json::to_string(txn) {
        ...
        Err(PlatformError::IoError(format!(
...
pub fn create_directory_and_rename_path(
    path_str: &str,
    overwrite: bool,
) -> Result<(), PlatformError> {
    let path = Path::new(&path_str);
