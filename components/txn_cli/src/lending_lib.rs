...
use zei::xfr::structs::{
    AssetRecordTemplate, AssetTracerMemo, BlindAssetRecord, OwnerMemo,
};
    ...
    T: RestfulLedgerUpdate + RestfulLedgerAccess,
{
    ...
    if let Some(file) = memo_file {
        ...
        .map_err(|e| PlatformError::ZeiError(error_location!(), e))?
        .ctexts;
        let tracer_memo = AssetTracerMemo {
    ...
    let (_, owner_memo) =
        load_blind_asset_record_and_owner_memo_from_file(fiat_txn_file)?;
    ...
    // Initiate loan
    let lender_template = AssetRecordTemplate::with_no_asset_tracking(
        amount,
        debt_code.val,
        AssetRecordType::NonConfidentialAmount_NonConfidentialAssetType,
        lender_key_pair.get_pk(),
    );
    let borrower_template = AssetRecordTemplate::with_no_asset_tracking(
        amount,
        fiat_code.val,
        AssetRecordType::NonConfidentialAmount_NonConfidentialAssetType,
    ...
    let mut txn_builder =
        TransactionBuilder::from_seq_id(rest_client.get_block_commit_count()?);
    ...
    T: RestfulLedgerAccess + RestfulLedgerUpdate,
{
    ...
    // Get templates
    let spend_template = AssetRecordTemplate::with_no_asset_tracking(
        amount_to_spend,
        fiat_code.val,
        AssetRecordType::NonConfidentialAmount_NonConfidentialAssetType,
        lender_key_pair.get_pk(),
    );
    let burn_template = AssetRecordTemplate::with_no_asset_tracking(
        amount_to_burn,
        debt_code.val,
        AssetRecordType::NonConfidentialAmount_NonConfidentialAssetType,
