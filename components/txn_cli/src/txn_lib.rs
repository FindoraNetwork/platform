...
use zei::xfr::structs::{
    AssetRecordTemplate, AssetTracingPolicies, AssetTracingPolicy, OpenAssetRecord,
    OwnerMemo, XfrAmount, XfrAssetType,
};
...
pub fn define_asset(
    ...
    let mut txn_builder = TransactionBuilder::from_seq_id(seq_id);
    txn_builder.add_operation_create_asset(
        issuer_key_pair,
        Some(token_code),
        asset_rules,
        &memo,
...
pub fn issue_and_transfer_asset(
    ...
    let output_template = AssetRecordTemplate::with_asset_tracking(
        amount,
        token_code.val,
        record_type,
    ...
    T: RestfulLedgerAccess + RestfulLedgerUpdate,
{
    ...
    let input_template = AssetRecordTemplate::with_no_asset_tracking(
        amount,
        code.val,
...
pub fn query_utxo_and_get_amount<T>(
    utxo: u64,
    rest_client: &T,
...
pub fn match_error_and_exit(error: PlatformError) {
    match error {
        ...
        PlatformError::IoError(io_error) => {
            ...
            if io_error.contains("File doesn't exist:")
                || io_error.contains("Failed to read")
            {
