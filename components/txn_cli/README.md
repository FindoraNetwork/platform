...
The `txn_cli` application is intended to help you experiment with the RESTful API and access the Ledger from scripts. The `txn_cli` provides a command line interface for constructing and submitting requests to the Ledger. The `txn_cli` application creates transactions and submits them to the ledger server.
...
* Examples below are assuming the current directory is `platform/target/debug`. If not, change `./txn_cli` to the path to `./txn_cli`.
...
* Assign to AIR
```
./txn_cli --txn txn_define asset_issuer --id 0 air_assign --address air_key --data 'Some data.'
```
