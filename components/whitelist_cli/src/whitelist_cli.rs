    ...
    T: RestfulLedgerAccess,
{
    ...
    match inputs.subcommand() {
        ...
        ("prove_and_verify_membership", Some(prove_and_verify_matches)) => {
            let index = if let Some(index_arg) =
                prove_and_verify_matches.value_of("index")
            {
            ...
            let utxo = if let Some(utxo_arg) = prove_and_verify_matches.value_of("utxo")
            {
                ...
                println!(
                    "Missing UTXO of the transaction which transferred the asset. Use --utxo."
                );
                return Err(PlatformError::InputsError(error_location!()));
            };
            let blind = if let Some(blind_arg) =
                prove_and_verify_matches.value_of("blind")
            {
                serde_json::from_str::<Scalar>(&blind_arg).map_err(|e| des_fail!(e))?
            } else {
                println!(
                    "Missing serialized blinding factor for the asset type code commitment. Use --blind."
                );
                return Err(PlatformError::InputsError(error_location!()));
            };
...
mod tests {
    ...
    fn test_cmd() {
        ...
        for code in codes.iter() {
            let (utxo, _, code_blind) = define_issue_transfer_and_get_utxo_and_blinds(
                &issuer_key_pair,
                &recipient_key_pair,
                10,
                *code,
        ...
        add_member_cmd(&codes[0].to_base64(), &mut ledger_standalone)
            .expect("Failed to set conversion rate.");
        add_member_cmd(&codes[1].to_base64(), &mut ledger_standalone)
            .expect("Failed to set conversion rate.");
        add_member_cmd(&codes[2].to_base64(), &mut ledger_standalone)
            .expect("Failed to set conversion rate.");
