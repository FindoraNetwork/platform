...
pub(crate) fn get_decompressed_commitment(
    ...
    commitment.decompress().ok_or_else(|| {
        ...
        match query_utxo_and_get_amount(utxo, rest_client)? {
            ...
            XfrAmount::Confidential((amount_commitment_low, amount_commitment_high)) => {
                ...
                let amount_and_code_blinds = AmountAndCodeBlinds::new(
                    calculate_amount_blinds(amount_blind_low, amount_blind_high),
                    code_blind,
                );
                let amount_commitment =
                    (get_decompressed_commitment(amount_commitment_low)?
                        + get_decompressed_commitment(amount_commitment_high)?
                            * Scalar::from(1u64 << 32))
                    .compress();
                let code_commitment = PublicParams::new()
                    .pc_gens
...
impl SolvencyAudit {
    ...
    pub fn verify_solvency(
        &self,
        account: &AssetAndLiabilityAccount,
    ) -> Result<(), PlatformError> {
...
mod tests {
    ...
    fn test_prove_solvency_fail() {
        ...
        let (mut hidden_assets, mut hidden_assets_blinds) = add_hidden_asset_amounts(
            &issuer_key_pair,
            recipient_key_pair,
            &mut account,
            &codes,
        ...
        let (mut hidden_liabilities, mut hidden_liabilities_blinds) =
            add_hidden_liability_amounts_smaller(
                &issuer_key_pair,
                recipient_key_pair,
                &mut account,
                &codes,
        ...
        match audit.prove_solvency_and_store(
            &mut account,
            &mut hidden_assets,
            ...
            unexpected_result => {
                panic!(format!(
                    "Expected ZeiError::SolvencyProveError, found {:?}.",
                    unexpected_result
                ));
            }
        }
    ...
    fn test_prove_and_verify_solvency_fail() {
        ...
        let (mut hidden_liabilities, mut hidden_liabilities_blinds) =
            add_hidden_liability_amounts_larger(
                &issuer_key_pair,
                recipient_key_pair,
                &mut account,
                &codes,
        ...
        match audit.verify_solvency(&account) {
            ...
            unexpected_result => {
                panic!(format!(
                    "Expected ZeiError::SolvencyVerificationError, found {:?}.",
                    unexpected_result
                ));
            }
        }
    ...
    fn test_prove_and_verify_solvency_simple_pass() {
        ...
        let recipient_key_pair = &XfrKeyPair::generate(&mut ChaChaRng::from_entropy());
        let prng = &mut ChaChaRng::from_entropy();
        let (utxo, amount_blinds, code_blind) = issue_transfer_and_get_utxo_and_blinds(
            issuer_key_pair,
            recipient_key_pair,
            10,
            code,
