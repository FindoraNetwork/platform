...
mod filters {
    use super::handlers;
    use super::models::{Db, ListOptions, Credential};
    use warp::Filter;
    ...
    pub fn credentials(
        ...
        credentials_list(db.clone())
            .or(credentials_at(db.clone()))
...
mod handlers {
    ...
    pub async fn create_credential(create: Credential, db: Db) -> Result<impl warp::Reply, Infallible> {
    ...
    pub async fn update_credential(
        ...
        let decoded_id = urldecode::decode(id); // Why can't I use it and call it as decode? (brian)
        log::debug!("update_credential: id={}, credential={:?}", decoded_id, update);
        let mut vec = db.lock().await;
