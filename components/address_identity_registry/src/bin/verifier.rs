...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let issuer_resp =
        reqwest::get(&format!("http://localhost:3030/issuer_pk/{}", &credname))
            .await?
            .json::<PubCreds>()
            .await?;
    ...
    let addr_and_pok: AIRAddressAndPoK = client
        .post(&req_string)
    ...
    let air_result: AIRResult = reqwest::get(&format!(
        "{}://{}:{}/air_address/{}",
    ...
    if let Some(commitment_string) = air_entry {
        ...
        if check_merkle_proof(
            ...
            if let Err(e) = credential_verify(
                ...
                Err(
                    format!("smt merkle proof succeeds but ac_verify fails with {}", e)
                        .into(),
                )
            } else {
