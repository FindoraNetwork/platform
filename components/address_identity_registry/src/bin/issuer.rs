...
mod handlers {
    ...
    pub async fn sign_credential(
        ...
        let attrs: Vec<(String, &[u8])> = user_creds
            .attrs
            .iter()
            .map(|(field, attr)| (field.clone(), attr.as_bytes()))
            .collect();
        match credential_sign(
...
mod models {
    ...
    use credentials::{
        credential_issuer_key_gen, CredIssuerPublicKey, CredIssuerSecretKey,
        CredUserPublicKey,
    };
    ...
    pub fn make_db() -> Db {
        ...
        let a = [
            (String::from("passport"), mk_passport_credkind(&mut prng)),
    ...
    fn mk_credkind(
        ...
        let (issuer_pk, issuer_sk) =
            credential_issuer_key_gen::<_>(&mut prng, attributes);
        CredentialKind {
